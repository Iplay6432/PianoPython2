...
import pyglet
from backend.thing import Note
...
class PianoKeyboard:
    def __init__(self, window):
        ...
        self.WHITE_KEY_HEIGHT = window.width / 2
        self.BLACK_KEY_HEIGHT = self.WHITE_KEY_HEIGHT * 0.64
        self.BORDER_WIDTH = window.width / 500
        self.keys = []
        White = ["C", "D", "E", "F", "G", "A", "B"]
        Black = ["Db", "Eb", "Gb", "Ab", "Bb"]
        Octive = [3, 4, 5]
        for i in range(self.OCTIVES * 7):  # white keys
            print(list[Note.White])
            print(type(Note.White))
            self.keys[i] = KeyboardNote(
                list(Note.White)[i % 7],
        ...
        for i in range(3):
            for t in range(2):
                ...
                m += 1
            temp = (
                self.WHITE_KEY_WIDTH * 5
                + n * self.BLACK_KEY_WIDTH
                + t * self.WHITE_KEY_WIDTH
                + i * (7 * self.WHITE_KEY_WIDTH)
            )
            for n in range(3):
